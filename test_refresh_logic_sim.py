...
# S3: [ticker] pattern - revenue-only, EPS+guidance+analyst N/A, model says Positive -> capped at Neutral
...
f = {}; inc, _ = _apply_result(f, "[ticker]", dict(view), None, 5.0)
check("S3 [ticker] revenue-only Positive capped at Neutral", f["[ticker]"]["sentiment"] == "Neutral")
check("S3 downgrade annotated PARTIAL", "auto-downgraded: PARTIAL" in f["[ticker]"]["reasoning"])
