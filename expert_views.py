...
def generate_expert_view(client, row_data, news_text=None, news_source=None, active_alerts_text=None, nvidia_api_key=None):
    ...
    def _pending_fallback(reason):
        ...
        [company] fallback isn't configured at all, or it was tried and also
    ...
    # 2. Fallback to DeepSeek V4 Flash via [company] API if configured
    if nvidia_api_key:
        try:
    ...
    # No [company] fallback configured -- surface the original Gemini error
