...
Each alert rule has its own day + hour schedule (set per-rule in the Alert Rules tab) — any of the 24 hours ET, no restriction. The workflow itself just runs **every hour, year-round** (a single `cron: "0 * * * *"`), and a cheap "gate" job (installs only `requests`, no full dependency set) asks `alerts.is_rule_due()` whether anything is actually due that hour before the expensive job (which fetches live prices) runs. In practice most hours find nothing due and exit in well under a minute. Because the workflow always listens every hour, changing a rule's schedule in the app is a pure config change — it never requires editing or re-pushing this workflow file, no matter which hour(s) you pick. `alert_state.json` (tracks what's already fired, so you don't get duplicate pings) persists between runs via `actions/cache`, already wired up.
...
Rough cost: 24 lightweight gate runs/day (a few seconds each) plus however many full checks your rules' schedules actually trigger comes in comfortably under GitHub's free 2,000 minutes/month for a private repo (or free either way on a public repo). One thing to know: GitHub auto-disables scheduled workflows after 60 days with no commits to the repo (it does email a heads-up first, sent to whoever last enabled the workflow) — a trivial commit (even just touching this README) resets that clock, so if you go quiet on the repo for ~2 months, either push something small or manually re-enable the workflow from the Actions tab. Also worth knowing: GitHub's scheduler is documented as best-effort — very frequent (sub-hourly) schedules are often skipped, but hourly schedules like this one are honored reliably in practice.
